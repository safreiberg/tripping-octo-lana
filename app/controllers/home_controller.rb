class HomeController < ApplicationController
  ...
  def auth
    ...
    @sleeps = current_user.sleeps.where('(end BETWEEN ? AND ?) OR (start BETWEEN ? AND ?)', DateTime.now.beginning_of_day, DateTime.now.end_of_day, DateTime.now.beginning_of_day, DateTime.now.end_of_day).all
